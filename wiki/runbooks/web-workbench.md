...
采用 Arctic editorial 视觉：冷灰全屏首屏、原创冰质素材、大字叠层，以及无卡片的需求和审批排版。首屏移除视角缩略图，以系统主题、雕塑和开始创作为重点。雕塑支持点击、横向拖动和左右方向键切换视角；桌面用冷白冰晶光标替代系统箭头。触摸使用原生交互；减少动态效果时关闭过渡、视差和拖尾。仍使用原有 HTML/CSS/JavaScript 和 Python 服务，无新增运行时依赖。素材与生成提示词见 `orchestration/assets/README.md`。先前绿色 Figma 稿是历史版本，不代表当前页面。
