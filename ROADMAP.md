...
- [x] 长期项目、客户标签与单次任务分离；旧归档增量迁移
- [x] 原哈希校验的增量修改与修复；未涉及文件保留
- [x] 独立候选、验收后提升、失败保留旧版、显式恢复与审计
- [x] 弹窗版本、差异、继续修改及回归测试
- [x] P0-2 单机成功检查点恢复、幂等新运行、重审与 OS 进程锁
- [x] P0-3 交付模式与可配置真实浏览器业务测试（桌面/手机正反例）
- [x] P1 调用/token/金额预留上限、单价快照与费用面板、去重累计统计
- [x] P1 已验证源码与证据的本地交付包
- [ ] P1 模板套餐、执行沙箱、真实服务器发布和生产回滚
- [ ] P2 订单、实收、维护与服务器迁移

## 2026-09-12 网页视觉优化

- [x] 说明书独立正文滚动区、三角光标与有限粒子拖尾，增加长文滚动和粒子消退回归

- [x] 修复宽屏/矮窗口首屏重叠，增加 11 尺寸布局回归

- [x] 恢复雕塑与右下角四阶段说明联动，保留无缩略图布局

- [x] 主题首屏、去除缩略图、冰晶光标与六章系统使用说明

- [x] Arctic editorial 首屏、原创冰质素材、开放式需求与审批排版
- [x] 四视角切换、克制视差、键盘与减少动态效果支持
- [x] 四尺寸浏览器视觉检查与审批/下载回归；固定静态素材路由边界测试

## 2026-09-10 网页指令工作流

- [x] 自动识别本地 DeepSeek 环境变量、隔离 provider 凭据与端点、运行时模型选择

- [x] 本地网页输入、进度查询、方案/预览审批、报告下载
- [x] 原始指令贯穿各 Agent 与修复；逐项要求代码证据核对
- [x] 默认构建门、最终复审、安全扫描前置、验收失败关闭
- [x] 保护生成配置、清理旧生成文件、独立运行目录
- [ ] 按 DAG 增量执行、业务 E2E 测试、容器隔离、生产部署与监控
...
| 18 | 生成文件路径安全：阻断目录穿越/绝对路径/符号链接越界，规范 npm 包名；构建门超时与 npm 缺失可诊断 | 2026-07-20 | - |
| 19 | 流水线韧性：统一 Runner 异常边界、终止状态，LLM 有限重试/超时，LangGraph 编译复用 | 2026-07-20 | - |
...
- 后端 SQLite 持久化任务、日志、审批与生成结果；重启保留历史，未完成标记中断。
- 导航弹窗浏览历史、查看详情与下载记录；显式重新启动符合条件的历史应用预览。
- 三角粒子光标放大约 25%。服务器部署与多用户认证仍为后续工作。
